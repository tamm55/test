...
/* ============================================================
   InNoVarr.Xxx — Optimized Script (Ultra Smooth)
   ============================================================ */
...
// Ẩn loader khi DOM + ảnh nền sẵn sàng (tối đa 2s)
...
/* ---------- 2. PARTICLES (tối ưu: giảm count, dùng offscreen ---------- */
const canvas = $('particle-canvas');
const ctx = canvas?.getContext('2d', { alpha: true });
let W = 0, H = 0, animId = null;
const PARTICLE_COUNT = isTouch ? 18 : 30; // giảm cho mobile
const particles = [];

function resizeCanvas() {
  if (!canvas) return;
  const dpr = Math.min(devicePixelRatio || 1, 2); // cap DPR để tránh quá tải
...
function animateParticles() {
  ...
  // Tắt shadow cho nhanh hơn (thay bằng màu sáng)
  ctx.shadowBlur = 0;
...
/* ---------- 3. STARS (giảm 80 → 30, dùng fragment) ---------- */
...
/* ---------- 6. SOCIAL CARDS (dùng template fragment) ---------- */
const socialData = [
  ...
  socialData.forEach((item, i) => {
    ...
    card.innerHTML = `
      <div class="social-card-inner">
        <div class="logo-svg"><img src="${item.img}" alt="${item.label}" loading="lazy" decoding="async"></div>
  ...
  if (!qrModal) {
    ...
    qrModal.innerHTML = `
      ...
      <div class="qr-bank-box" role="dialog" aria-modal="true" aria-label="QR Bank">
        ...
        <img src="image/qr-bank.png" alt="QR Bank" loading="lazy">
      </div>`;
...
/* ---------- 8. TILT 3D (tối ưu: chỉ desktop, dùng pointermove) ---------- */
...
/* ---------- 10. ORIENTATION (chỉ 1 handler duy nhất) ---------- */
